/* Which users are the most active (by the criteria: how many minutes did they log)? */
/* Counts the active days, sums the different types of minutes the user logged. The table is grouped by each user's entries*/
...
/* Formats the dates in "YYYY-MM-DD" format to days of the week as string format */
...
/*How does one user record their activities compared to the other users?*/
...
/* This additional query found nulls in the above table. If there were nulls, it meant that there was a record in the cohesive table that wasn't in the separate weight or sleep table. The output was empty, as expected */
SELECT Id, ActivityDate
FROM `bellabeat-case-study-384718.DailyActivity.joined_activity_weight_sleep`
WHERE Id IS NULL
LIMIT 1000
  
/* How many days did each user log their sleep or weight? What is the percentage of those days for each user?*/
...
/*At what time during a 24-hour period did the users log their steps. When did they log the most steps? The least steps? */
...
FROM
  /* Uses CASE to separate the 24-hour times into four sections*/
...
FROM(
/* Converts the datetime into 24-hour times*/
